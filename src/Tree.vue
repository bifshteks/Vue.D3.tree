...
export default {
    ...
    methods: {
        ...
        updateGraph(source, {transitionDuration = undefined, resetPopUp = true} = {}) {
            ...
            const origin = currentPosition ? currentPosition.get(correctedSource.id) : {
                x: correctedSource.x0,
                y: correctedSource.y0
            }
...
// API
        ...
        expandAll(d, update = true) {
            onAllChilddren(d, child => {
                this.expand(child, false)
            })
        ...
        showOnly(d) {
            ...
            const mapped = {}
            shouldBeRetracted.filter(node => node.children).forEach(rectractedNode => rectractedNode.each(c => {
